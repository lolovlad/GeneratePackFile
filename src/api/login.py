...
from ..models.UserLogin import UserLogin, Token, UserSigIn, UserSignup
...
async def sign_in(user_sign_in: UserSignup,
                  login_services: LoginServices = Depends()):

    user = await login_services.registrate_user(user_sign_in)
    return user
